const enLabels = {
  ...
  productionLabel: "Phone/camera module",
...
const productionNotes = {
  raw: {
    en: "Sensor + MIPI RAW input. In a phone camera, this is the Bayer frame delivered by the image sensor before ISP hardware or software turns it into a viewable image.",
    zh: "对应传感器 + MIPI RAW 输入。在手机相机里，这是图像传感器送进 ISP 前的 Bayer 帧，还不是可直接显示的照片。"
  },
  norm: {
    en: "Sensor front-end / black clamp / linearization. Production pipelines calibrate optical black rows, dark current, and pedestal before any color processing.",
    zh: "对应 Sensor Front-End、黑电平钳位和线性化。生产链路会先校准 optical black、暗电流和 pedestal，再进入颜色处理。"
  },
  bad: {
    en: "Defect pixel correction, often driven by factory defect maps plus runtime hot-pixel detection. This is usually early in RAW-domain ISP.",
    zh: "对应坏点校正模块，通常结合工厂 defect map 和运行时热像素检测，属于 RAW 域前段处理。"
  },
  lsc: {
    en: "Lens shading correction mesh. Mobile camera tuning uses per-module calibration tables to compensate lens falloff and color shading.",
    zh: "对应 LSC 网格补偿。手机相机调试会使用模组标定表，补偿镜头像场亮度衰减和色彩阴影。"
  },
  demosaic: {
    en: "RAW-to-RGB reconstruction. Real ISPs often use edge-aware demosaic to reduce zipper artifacts, false color, and moire.",
    zh: "对应 RAW 到 RGB 重建。真实 ISP 常使用边缘感知去马赛克，减少拉链、伪色和摩尔纹。"
  },
  wb: {
    en: "AWB gains from the 3A system. Phone cameras estimate illuminant color, apply channel gains, and often smooth gains across video frames.",
    zh: "对应 3A 系统中的 AWB 增益。手机会估计光源颜色、施加通道增益，并在视频帧间平滑变化。"
  },
  ccm: {
    en: "Color calibration / color science. Camera RGB is mapped toward sRGB or a target color space using calibrated matrices and scene-dependent tuning.",
    zh: "对应色彩标定和 color science。相机 RGB 会通过标定矩阵和场景化调参映射到 sRGB 或目标色彩空间。"
  },
  denoise: {
    en: "RAW/RGB/YUV noise reduction. Production cameras combine spatial, temporal, and AI denoise, especially in night and video modes.",
    zh: "对应 RAW/RGB/YUV 降噪。生产相机会结合空域、时域和 AI 降噪，尤其用于夜景和视频模式。"
  },
  sharpen: {
    en: "Detail enhancement / edge enhancement. Phone pipelines tune sharpening with denoise, texture protection, halo suppression, and face/detail priorities.",
    zh: "对应细节增强和边缘增强。手机 pipeline 会联合降噪、纹理保护、光晕抑制、人脸/细节优先级一起调。"
  },
  tone: {
    en: "AE result, tone mapping, gamma, local contrast, and display/output conversion. This is where RAW-domain signal becomes user-facing JPEG/preview RGB.",
    zh: "对应 AE 结果、tone mapping、Gamma、局部对比和显示/输出转换。这里把 RAW 域信号变成用户看到的预览或 JPEG/RGB。"
  }
};
...
function localizeStep(step, language) {
  ...
  localized.production = localize(productionNotes[step.key], language);
  return localized;
}
